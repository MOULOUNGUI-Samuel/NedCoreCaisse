    ...
    <div class="card">
        <div class="card-header">
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="card-title">Mouvement récent de la caisse - {{ $activeCaisse->nomcaisse }}</h4>
                </div>
            </div>
        </div>
                        ...
                        @forelse ($mouvements as $mvt)
                            ...
                            <tr class="align-middle bg-white" style="border-bottom: 1px solid #dee2e6;">
                                ...
                                <td class="text-center">
                                    ...
                                    <span
                                        class="fw-bold text-dark">{{ \Carbon\Carbon::createFromFormat('YmdHisu', $mvt->datemvtcaisse)->format('d M Y à H:i') }}</span>
                                </td>
                                <td>
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="text-dark fw-semibold">{{ $mvt->motif_mouvement_caisse ?? 'Aucun motif' }}</span><br>
                                        <small class="text-muted">{{ $mvt->remarquemvtcaisse ?? '' }}</small>
                                    </div>
                                </td>
                                <td class="text-end fw-semibold">
                                    <span class="badge bg-transparent text-dark fs-12 mb-2">Montant
                                        débit</span><br>
                                    <span
                                        class="text-danger fw-semibold">{{ number_format($mvt->montant_debit ?? 0, 2, ',', ' ') }}</span>
                                </td>
                                <td class="text-end text-muted">
                                    <span class="badge bg-transparent text-dark fs-12 mb-2">Montant
                                        crédit</span><br>
                                    <span
                                        class="text-dark fw-semibold">{{ number_format($mvt->montant_credit ?? 0, 2, ',', ' ') }}</span>
                                </td>
